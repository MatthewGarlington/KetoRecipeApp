...
struct RecipeDetailView96: View {
    
    
    
    let KetoJSONDecodedDetail: [RecipeArray] = Bundle.main.decode("KetoRecipeDetail.json")
    let KetoJSONDecodedHealth: RecipeHealth? = Bundle.main.decode("RecipeHealth96.json")
        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 0) {
                ...
                ZStack(alignment: .bottomLeading) {
                    ...
                    HStack(spacing: 75) {
                        Text("\(KetoJSONDecodedDetail[96].title ?? "")")
                            .foregroundColor(.white)
                            .bold()
                            .frame(width: 210, height: 100)
                        
                        
                        Text("Health Score: \(KetoJSONDecodedDetail[96].healthScore ?? 0)")
                ...
                ZStack(alignment: .leading) {
                    ...
                    VStack(alignment: .leading, spacing: 0) {
                        ...
                        VStack(alignment: .leading, spacing: 5) {
                            ...
                            Text("\(KetoJSONDecodedDetail[96].extendedIngredients[0].original ?? "")")
                                .font(.system(size: 15, weight: .semibold))
                            
                            
                            
                            
                            Text("\(KetoJSONDecodedDetail[96].extendedIngredients[1].original ?? "")")
                                .font(.system(size: 15, weight: .semibold))
                            
                            
                            
                            Text("\(KetoJSONDecodedDetail[96].extendedIngredients[2].original ?? "")")
                                .font(.system(size: 15, weight: .semibold))
                            
                            
                            
                            Text("\(KetoJSONDecodedDetail[96].extendedIngredients[3].original ?? "")")
                                .font(.system(size: 15, weight: .semibold))
                            
                            
                            Text("\(KetoJSONDecodedDetail[96].extendedIngredients[4].original ?? "")")
                                .font(.system(size: 15, weight: .semibold))
                            
                            
                            Text("\(KetoJSONDecodedDetail[96].extendedIngredients[5].original ?? "")")
                                .font(.system(size: 15, weight: .semibold))
                    ...
                    VStack(alignment: .leading, spacing: 20) {
                        ...
                        HStack {
                            Text("Total Time:")
                                .font(.system(size: 20, weight: .semibold))
                                .padding(.horizontal)
                            Text("\(KetoJSONDecodedDetail[96].readyInMinutes?.formatNumber() ?? "") mins")
                                .font(.system(size: 20, weight: .semibold))
                        }
                ...
                Link(destination: URL(string: "\(KetoJSONDecodedDetail[96].sourceUrl)")!, label: {
...
struct RecipeDetailView96_Previews: PreviewProvider {
